...
[//]: # (source: [developer.android.com](https://developer.android.com/guide/playcore/asset-delivery))
...
If you want to jump directly to implementing PAD in your game, see [Next step](#next-step-instructions).

![Distribution Icon](https://developer.android.com/static/images/picto-icons/distribution.svg)

#### Single publishing artifact

Publish a single artifact to Play including all your game's resources

![Pathway Icon](https://developer.android.com/static/images/picto-icons/pathway.svg)

#### Flexible delivery modes

Control when and how Play delivers your game assets

![Frame Icon](https://developer.android.com/static/images/picto-icons/frame.svg)
...
Asset packs configured as `fast-follow` and `on-demand` are served as archive files by the Google Play Store (and not as split APKs). These packs are then expanded in the app's internal storage. You can query the location of asset packs served this way using the [Play Asset Delivery Library](/guide/playcore#java-kotlin-asset-delivery). The app can't assume the existence of these files or their locations because these files may be deleted by the user or moved by the Play Asset Delivery Library across play sessions. Even though these files are writable by the app, you should treat them as read-only since asset pack patches depend on the integrity of these files. These packs do not contribute to the app size listed on the Google Play Store.

When using Play Asset Delivery in an [instant app](/gtopic/google-play-instant/overview), on-demand is the only supported mode.

**Note:** To successfully install or update an app, Android requires a certain amount
of free disk space on the device. `install-time` asset packs have the same
requirements as the base APK, which **require at least two times the size of all the
asset packs**. On the other hand, `fast-follow` and `on-demand`
asset packs require only a few hundred extra MBs.

Minimize the size of your `install-time` asset packs to
avoid the situation where users have to free up space to install or update your apps.
...
1. The patch for the app, including all assets, is downloaded to a secure
...
4. The patch for the assets is copied and applied to assets stored in the app's
   internal storage.
...
If you use [Texture Compression Format Targeting](/guide/app-bundle/asset-delivery/texture-compression), these download limits apply separately to each unique texture format.

## Next step

Build Play Asset Delivery into your game or app using one of the following:

[Get started for Java](/guide/app-bundle/asset-delivery/build-native-java) [Get started for Native](/guide/app-bundle/asset-delivery/build-native-java) [Get started for Unity](/guide/app-bundle/asset-delivery/build-unity)

## Terms of service and data safety

By accessing or using the Play Asset Delivery Library, you agree to the [Play Core Software Development Kit Terms of Service](/guide/playcore#license). Please read and understand all applicable terms and policies before accessing the library.
...
[![Devsisters Pad Thumbnail Image](https://developer.android.com/static/images/distribute/stories/Devsisters_PAD_thumbnail.png)](https://developer.android.com/stories/games/devsisters)
...
[![Cat Daddy Games Logo Image](https://developer.android.com/static/images/cards/distribute/stories/cat-daddy-games-logo.png)](https://developer.android.com/stories/games/cat-daddy)
...
[![Cdpr Thumbnail Image](https://developer.android.com/static/images/cards/distribute/stories/cdpr-thumbnail.png)](https://developer.android.com/stories/games/cdpr)
...
[![Puzzle Kids Framed Image](https://developer.android.com/static/images/cards/distribute/stories/puzzle-kids-framed.png)](https://developer.android.com/stories/games/rv-appstudios-pad)
...
[![Gameloft Asphalt8 Image](https://developer.android.com/static/images/cards/distribute/stories/gameloft-asphalt8.jpg)](https://developer.android.com/stories/games/gameloft-pad)
