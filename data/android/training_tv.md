# Android TV overview Stay organized with collections Save and categorize content based on your preferences.

[//]: # (source: [developer.android.com](https://developer.android.com/training/tv))

If you've got an Android app or game, Android TV can bring it to your users in
their living room. Android TV apps use the same architecture as those for phones
and tablets. This approach means you can build new TV apps based on what you
already know about building apps for Android, or extend your existing apps to
also run on TV devices.

## Documentation

* Designing TV apps:
  + [Design TV UI ⍈](/design/ui/tv)
* Building TV apps:
  + [Get started with TV apps](/training/tv/get-started)
  + [Build adaptive apps](/training/tv/build-adaptive-apps)
  + [Build TV playback apps](/training/tv/playback), adjust the [audio](/training/tv/playback/audio-capabilities) and
    [display](/training/tv/playback/framerate) settings for optimal playback, and [help users find your
    content on TV](/training/tv/discovery)
  + [Build TV games](/training/tv/games)
  + [Build TV input services](/training/tv/tif)
  + [Accessibility best practices](/training/tv/accessibility)
  + [Memory optimization best practices](/training/tv/playback/memory)
* Publishing TV apps:
  + [TV Apps checklist](/training/tv/publishing/checklist)
  + [Distributing TV apps on Google Play](/training/tv/publishing/distribute)
* Android TV OS releases:
  + [Android 12 for TV](/tv/release/12)
  + [Android 13 for TV](/tv/release/13)
  + [Android 14 for TV](/tv/release/14)
  + [Android 16 for TV](/tv/release/16)

## Additional resources

To learn more about Android TV, see the following additional resources.

### Samples

### Codelabs

### Blogs

### Videos
