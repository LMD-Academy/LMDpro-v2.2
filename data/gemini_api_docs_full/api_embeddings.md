...
[//]: # (source: [ai.google.dev](https://ai.google.dev/api/embeddings))
...
| JSON representation |
| --- |
| ``` {   "metadata": {     object   },    // output   "error": {     object (google.rpc.Status)   },   "response": {     object (EmbedContentResponse)   }   // Union type } ``` |
