# Process input and output data with the LiteRT Support Library Stay organized with collections Save and categorize content based on your preferences.

[//]: # (source: [ai.google.dev](https://ai.google.dev/edge/litert/android/metadata/lite_support))

[**Introducing Google AI Edge Portal**](https://ai.google.dev/edge/ai-edge-portal): Benchmark Edge AI at scale. [Sign-up](https://docs.google.com/forms/d/e/1FAIpQLSfTcGPycQve8TLAsfH46pBlXBZe9FrgJAClwbF7DeL1LgVn4Q/viewform) to request access during private preview.

**Note:** LiteRT Support Library currently only supports Android.

Mobile application developers typically interact with typed objects such as
bitmaps or primitives such as integers. However, the LiteRT interpreter API that
runs the on-device machine learning model uses tensors in the form of
ByteBuffer, which can be difficult to debug and manipulate. The [LiteRT Android
Support
Library](https://github.com/tensorflow/tflite-support/tree/master/tensorflow_lite_support/java)
is designed to help process the input and output of LiteRT models, and make the
LiteRT interpreter easier to use.
...
add the LiteRT library to the module’s `build.gradle` file:
...
Explore the [LiteRT Support Library AAR hosted at
...
The LiteRT Support Library has a suite of basic image manipulation methods such
as crop and resize. To use it, create an `ImagePreprocessor` and add the
required operations. To convert the image into the tensor format required by the
LiteRT interpreter, create a `TensorImage` to be used as input:
...
`DataType` of a tensor can be read through the [metadata extractor
library](https://developers.google.com/edge/litert/conversion/tensorflow/metadata#read_the_metadata_from_models) as
well as other model information.

### Basic audio data processing

The LiteRT Support Library also defines a `TensorAudio` class wrapping some
basic audio data processing methods. It's mostly used together with
...
See more details
[here](https://developers.google.com/edge/litert/conversion/tensorflow/metadata#normalization_and_quantization_parameters)
about normalization and quantization.
...
The quantization parameters of a tensor can be read through the [metadata
extractor
library](https://developers.google.com/edge/litert/conversion/tensorflow/metadata#read_the_metadata_from_models).
