# Get started with microcontrollers Stay organized with collections Save and categorize content based on your preferences.

[//]: # (source: [ai.google.dev](https://ai.google.dev/edge/litert/microcontrollers/get_started))
...
example is designed to demonstrate the absolute basics of using LiteRT
...
2. [Run inference](#run_inference) (in C++ 17): An end-to-end unit test that
   runs inference on the model using the [C++ library](https://developers.google.com/edge/litert/microcontrollers/library).
...
[Hello
World README.md](https://github.com/tensorflow/tflite-micro/tree/main/tensorflow/lite/micro/examples/hello_world/README.md)

The following sections walk through the example's
[`evaluate_test.cc`](https://github.com/tensorflow/tflite-micro/tree/main/tensorflow/lite/micro/examples/hello_world/evaluate_test.py),
...
To use the LiteRT for Microcontrollers library, we must include the
...
* [`schema_generated.h`](https://github.com/tensorflow/tflite-micro/tree/main/tensorflow/lite/schema/schema_generated.h)
  contains the schema for the LiteRT
...
* [`version.h`](https://github.com/tensorflow/tensorflow/blob/master/tensorflow/lite/version.h)
  provides versioning information for the LiteRT schema.
...
In order to create a unit test, we include the LiteRT for
...
The enum value `kTfLiteFloat32` is a reference to one of the LiteRT
