...
[//]: # (source: [ai.google.dev](https://ai.google.dev/edge/litert/next/compiler_plugin))
...
1. You are targeting a **new hardware backend** that is not supported.
2. You want to **offload specific model operations** to that hardware
   accelerator for performance or power efficiency.
3. You require support for AOT compilation (on workstation) or on-device
   compilation.

The plugin acts as a bridge, taking portions of the machine learning model and
converting them into a format that your target hardware can execute, using a
call to the backend's compiler. LiteRT bundles the custom bytecode generated by
the plugin into the `.tflite` model, making it executable using the LiteRT
runtime.
...
means of calling into the HAL given compiler output. For more details, refer to
the [dispatch documentation](https://developers.google.com/edge/litert/next/dispatch).

### AOT versus On-Device

LiteRT can use compiler plugins to support AOT compilation through our tooling,
as well as on-device compilation. on-device compilation is more flexible, fully
internalized within the LiteRT runtime API's and only requires the management of
a single model. The AOT flow can unblock compilation when it is too resource
intensive to run on-device which may be the case with many contemporary large
models.

### Fallback

LiteRT is built with support for heterogeneous graphs. Any operation not
selected by the plugin will be left to the CPU or made available for
...
function returns its output using the out-parameter **`LiteRtCompiledResult`**.
...
`sub`, and a specific composite op) only if all inputs and outputs are 32bit
floats. Usually determining whether or not an operation should be selected will
include a call to a validation hook in backend's compiler toolchain.
...
LiteRT ops to types to the backend compiler library. The functional example
plugin's "compilation" creates a human readable string which encodes the graph.
...
executing the result, and validating/benchmarking. Refer to the [accelerator
test suite documentation](https://developers.google.com/edge/litert/next/ats) and the [benchmarking and profiling
documentation](https://developers.google.com/edge/litert/next/benchmark).
