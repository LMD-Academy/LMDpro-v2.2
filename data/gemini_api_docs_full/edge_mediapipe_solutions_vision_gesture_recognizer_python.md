# Gesture recognition guide for Python Stay organized with collections Save and categorize content based on your preferences.

[//]: # (source: [ai.google.dev](https://ai.google.dev/edge/mediapipe/solutions/vision/gesture_recognizer/python))
...
parameters.**Note:** If you use the video mode or live stream mode, Gesture Recognizer uses tracking
to avoid triggering palm detection model on every frame, and this helps to
reduce the latency of Gesture Recognizer.
...
The Gesture Recognizer uses the recognize, recognize\_for\_video and recognize\_async
...
* When running in the video mode or the live stream mode, you must also
  provide the Gesture Recognizer task the timestamp of the input frame.
* When running in the image or the video model, the Gesture Recognizer task will
...
* When running in the live stream mode, the Gesture Recognizer task doesn’t block
  ...
  task is busy processing another frame, the task will ignore the new input
  frame.
