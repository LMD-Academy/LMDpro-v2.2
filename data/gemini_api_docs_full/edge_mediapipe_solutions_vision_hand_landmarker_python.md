# Hand landmarks detection guide for Python Stay organized with collections Save and categorize content based on your preferences.

[//]: # (source: [ai.google.dev](https://ai.google.dev/edge/mediapipe/solutions/vision/hand_landmarker/python))
...
**Note:** If you use the video mode or live stream mode, Hand Landmarker uses tracking to
...
The Hand Landmarker uses the detect, detect\_for\_video and detect\_async
...
* When running in the video mode or the live stream mode, you must also
  provide the Hand Landmarker task the timestamp of the input frame.
* When running in the image or the video model, the Hand Landmarker task will
...
* When running in the live stream mode, the Hand Landmarker task doesn’t block
  ...
  listener with the detection result every time it has finished processing
  an input frame. If the detection function is called when the Hand Landmarker
