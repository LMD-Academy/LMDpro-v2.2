# Pose landmark detection guide for Python Stay organized with collections Save and categorize content based on your preferences.

[//]: # (source: [ai.google.dev](https://ai.google.dev/edge/mediapipe/solutions/vision/pose_landmarker/python))
...
**Note:** If you use the video mode or live stream mode, Pose Landmarker uses tracking to
avoid triggering the model on every frame, which helps reduce latency.
...
* When running in the video mode or the live stream mode, also
  provide the Pose Landmarker task the timestamp of the input frame.
* When running in the image or the video model, the Pose Landmarker task
...
* When running in the live stream mode, the Pose Landmarker task returns
  ...
  input frame. If the detection function is called when the Pose Landmarker task
