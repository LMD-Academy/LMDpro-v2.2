...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/agent-environment))
...
You can also mount a `.agents/hooks.json` configuration file and custom interception scripts into the sandbox to enforce security guardrails or run automated validations whenever tools execute. For schema definitions and code examples, see [Hooks](/gemini-api/docs/agent-hooks).
...
* **Environment expiration:** Inactive offline environments are retained for 7
  days before expiring using automatic TTL cleanup. Passing an expired or
  invalid environment ID returns a `404 Not Found` error.
...
* [Antigravity Agent](/gemini-api/docs/antigravity-agent): Explore capabilities, tools, model selection, and pricing for the default agent.
* [Building Custom Agents](/gemini-api/docs/custom-agents): Define your own agents using `AGENTS.md` and `SKILL.md`.
* [Hooks](/gemini-api/docs/agent-hooks): Enforce security guardrails and run side-effect validations inside the sandbox.
