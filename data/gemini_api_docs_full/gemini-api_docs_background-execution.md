...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/background-execution))
...
Background execution is supported for standard Gemini models (such as `gemini-3.6-flash` and `gemini-3.1-pro-preview`) and Managed Agents (such as `antigravity-preview-05-2026`).
...
2. **Environment Parameter for Managed Agents:** When chaining interactions for Managed Agents (such as `antigravity-preview-05-2026`), requests must include both `previous_interaction_id` and `environment`.
