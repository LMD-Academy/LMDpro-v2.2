...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/billing))
...
| Usage tier | Qualification | [Billing tier cap](#spend-caps) |
| --- | --- | --- |
| **Free** | [Active project](/gemini-api/docs/api-key#google-cloud-projects) or free trial | N/A |
| **Tier 1** | [Set up and link an active billing account](#setup-billing) | $250 |
...
[certain models](/gemini-api/docs/pricing) in the Gemini API and AI Studio,
...
See [Deploying from Google AI Studio](/gemini-api/docs/aistudio-deploying) for
...
[link a billing account](#setup-billing) and [Prepay](#prepay) to move to the
Paid Tiers.
Then, you'll move through higher tiers based on cumulative spend and account
age. At Tier 3, you may have the option to switch to [Postpay](#postpay)
billing.

Tiers, rate limits, and billing account caps are all determined at the [billing
account](#cloud-billing) level.

\* *Enterprise-grade data privacy: For more information on data use
for paid services, see the [Terms of Service](/gemini-api/terms#data-use-paid).*
...
and [prepaying](#prepay) to add a minimum of $10 (or equivalent in other
currencies) of credits to your account.

1. Go to the AI Studio [API keys](https://aistudio.google.com/api-keys) page,
   [Projects](https://aistudio.google.com/projects) page, or anywhere you see
   **Set up billing** button in AI Studio.
   * New users will have a [project and API key](/gemini-api/docs/api-key#google-cloud-projects) created for them by default.
   * If you need a new key, click [**Create API key**](https://aistudio.google.com/api-keys)
     and follow the dialog to add a key-project pair to the table.
2. Find the Free Tier project you want to upgrade to the Paid Tier, and click
   **Set up billing** under the *Billing Tier* column.
...
4. If you have set up Google billing accounts in the past:
   ...
   * If you don't want to use any of your existing accounts, click **Add new
     billing account** and fill in or confirm your contact information and
     payment method to continue.
5. Next, you will either be:
   * Asked to prepay a minimum of $10 to complete billing
     setup (meaning your account is auto-assigned to the [Prepay](#prepay) billing
     plan),
   * Given the choice between [Prepay](#prepay) and [Postpay](#postpay)
     billing plans for your account.
   * Assigned to a [Postpay](#postpay) billing plan for an intermediary period
     until the new Prepay system propagates to all users (starting March 23, 2026).
6. After prepaying or selecting Postpay, your account setup is complete.

### Upgrade to the next paid tier

If you're already on a paid tier and meet the [criteria](#about-billing)
for a plan change, you will be automatically upgraded to the next tier
(subject to [processing times](#processing-times)).

## Verify billing status

After you have [linked a billing account](#setup-billing) to your project, you
...
positive [Prepay](#prepay) credit balance.
...
* "***Set up Prepay***" if the project does have a billing account attached, but
  is required to use a [Prepay](#prepay) billing plan that needs to be set up.
* "***No credits***" if the billing account is required to purchase
...
- Accounts that predate the introduction of Prepay and Postpay billing plans will
  be evaluated and assigned a plan or offered the choice between plans.
- Accounts created after the effective date might not be immediately affected;
...
your Prepay credit balance in [near real-time](#processing-times).
You can prepay by [adding credits](#buy-credits) into your account, or setting
up [auto-reload](#auto-reload). After credits are purchased, unused credits
expire after 12 months and are [non-refundable](#refunds), except after
[switching to a Postpay account](#postpay).
...
**Note:** Prepay billing is subject to overages during the approximately 10 minute
billing pipeline latency. Long-running tasks like
[batch mode](/gemini-api/docs/batch-api) and agents may continue to consume
credits beyond your balance before the system can process and halt usage.
...
project's billing details](#verify-billing) before continuing to use the Gemini
API.
...
**Note:** Prepay users must [purchase Prepaid credits](#buy-credits) before they can
...
billing account hits $0, Google Cloud credits will no longer be consumed.
...
[automatically assigned spend cap](#tier-spend-caps) based on your account tier.
...
When you meet the [Tier 3 criteria](#about-billing), you can
...
When [setting up billing](#setup-billing) for a new project, if you are eligible
for Postpay, you will have the option to choose between Prepay and Postpay in
the [billing setup](#setup-billing) dialogue.
...
**Experimental:** The feature is experimental and limited in scope. You are subject
to overages for around a 10 minute latency period.

You can set your own [project-level](/gemini-api/docs/api-key#google-cloud-projects) spend caps in AI Studio.
...
For details on the specific Google Cloud IAM permissions required to view or edit spend caps and billing information in AI Studio, see the [AI Studio troubleshooting guide](/gemini-api/docs/troubleshoot-ai-studio#iam-permissions).
...
Long-running tasks like [batch mode](/gemini-api/docs/batch-api) completions and
...
Each [tier](#about-billing) has a maximum monthly spend limit:
...
Monthly usage caps are enforced for the Gemini API at the [billing account](#cloud-billing) level. While default limits are preset, you can [request an
...
Account tier spend caps](#tier-spend-caps) will impact your ongoing projects,
follow these steps:
...
1. In the Google Cloud console, view your [Cloud Billing account Reports](https://console.cloud.google.com/billing/reports) page.
   * If you have more than one billing account, at the prompt, choose the Cloud
     Billing account for which you'd like to view cost reports.
2. The report defaults to "Group by Service" in the "Current month". You will
   see **Gemini API** in the **Service** column and total spend in the **Usage
   cost** column of the table.
3. To see granular costs limited to Gemini API usage, set the **Group by**
   filter to group by **SKU**, and the **Services** filter to **Gemini API**.
4. Adjust the **Time range by usage date** filter to your desired range to
   evaluate your historical spend in a period.

## Processing times

Billing signals and updates don't always happen in real time.

* **Credit usage**: Usage costs are typically drawn down from your balance
  within minutes.
* **Payment confirmation**: While most card payments are instant, some forms
  of payment (like bank transfers) may take several days to clear. Services
  only resume or upgrade after the purchase of credits is officially confirmed.
* **Tier upgrades**: After a successful payment, or when you meet the
  [upgrade criteria](#about-billing), tier upgrades usually reflect within 10
  minutes.
* **Total Cost breakdown graphs**: The graphs showing your total cost
  breakdown on both the [Billing](https://aistudio.google.com/billing) page
  and the [Spend](https://aistudio.google.com/spend) page can take up to 24
  hours to update.
...
meet the [criteria](#about-billing) and [manually upgrade](#postpay)
...
can [set up directly in AI Studio](#setup-billing).
You can use AI Studio to track spending, understand costs, and make payments.

**Note:** New users might be granted a [$300 Welcome credit](https://docs.cloud.google.com/billing/docs/in-product-billing-setup#welcome-credits)
in their new Google Cloud Billing accounts. If you opened your account
**after March 2, 2026**, you can't use these credits to pay for Gemini API and
AI Studio usage, but you can use them on [other Google Cloud products](https://docs.cloud.google.com/free/docs/free-cloud-features) in the
...
All [projects](/gemini-api/docs/api-key#google-cloud-projects) linked to a Cloud
...
[tier qualifications](#about-billing).
...
[API keys](/gemini-api/docs/api-key) are credentials generated inside a project.
...
[tier requirements](/gemini-api/docs/rate-limits#usage-tiers).
...
billing**](#setup-billing) in Google AI Studio. This will walk you through
...
Use the [`GenerativeModel.count_tokens`](/api/python/google/generativeai/GenerativeModel#count_tokens)
method to count the number of tokens. Refer to the [Tokens guide](/gemini-api/docs/tokens) to learn more about tokens.
...
Prepay users must first [purchase Prepaid credits](#buy-credits) before any
...
[Google Cloud Welcome credit](#cloud-credits), can be used towards Gemini API
and AI Studio.
...
Refer to the [Terms of service](/gemini-api/terms#paid-services) for details on
...
Prepaid billing plan. During the AI Studio [Set up Billing](#setup-billing)
...
You can [purchase credits](#buy-credits) on the AI Studio Billing page. During
...
Yes, we recommend that you configure [auto-reload](#auto-reload) in the AI
...
Yes, Prepay users can set a [Monthly Auto-Charge Limit](#monthly-auto-charge-limit)
...
[refund policy for Prepay accounts](#refunds).
...
additional credits](#buy-credits). After you've purchased credits, you should be
able to use the Gemini API. Note that there might be a [delay](#processing-times) while our systems update to reflect your credit balance.
...
You may have hit the [usage limit](#tier-spend-caps) for your current tier.
...
Cloud Billing account](#missed-payment).
...
[delays](#processing-times) in our ability to cut off usage after you consume
...
[auto-reload](#auto-reload) to automatically purchase more credits when your
...
When you establish a payment history and [reach a tier eligible](#about-billing)
...
When you upgrade to [Postpay](#postpay), Cloud Billing closes your Prepay
payments account, turns off [auto-reload](#auto-reload), and automatically
...
which case you can [activate billing](#setup-billing) in AI Studio to become
