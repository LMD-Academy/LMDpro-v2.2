...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/deep-research))
...
Research tasks involve iterative searching and reading and can take several minutes to complete. You must use [background execution](/gemini-api/docs/background-execution) (set `background=true`)
to run the agent asynchronously and poll for results or stream updates. See
[Handling long-running tasks](#long-running-tasks) for more details.

**Preview:** The Gemini Deep Research Agent is currently in preview. The Deep
Research agent is exclusively available using the [Interactions
API](/gemini-api/docs). You cannot access it through
`generate_content`.
...
Give the agent access to your own data by using the [File Search](/gemini-api/docs/file-search) tool.
...
`in_progress` to `completed` or `failed`. For a comprehensive guide on managing background tasks, see [Background execution](/gemini-api/docs/background-execution).
...
**Note:** The streaming connection can drop or expire. Since Deep Research tasks can
...
* **Prompt for unknowns:** Instruct the agent on how to handle missing data.
  For example, add *"If specific figures for 2025 are not available,
...
* **Use collaborative planning:** For complex queries, enable collaborative
  planning to review and refine the research plan before execution.
...
* **Custom tools:** You cannot currently provide custom Function Calling tools
  but you can use remote MCP (Model Context Protocol) servers with the Deep Research agent.
* **Structured output:** The Deep Research Agent currently
...
* **Google search:** [Google
  Search](/gemini-api/docs/google-search) is enabled by
  default and [specific
  restrictions](/gemini-api/terms#use-restrictions2)
  apply to the grounded results.

## What's next

* Learn more about the [Interactions API](/gemini-api/docs/interactions-overview).
* Learn how to use your own data using the [File Search](/gemini-api/docs/file-search)
  tool.
