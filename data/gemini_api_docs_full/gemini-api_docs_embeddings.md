...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/embeddings))
...
multiple inputs. You can find more information in [Embedding aggregation](#embedding-aggregation),
or you can use the [Batch API](/gemini-api/docs/batch-api#batch-embedding) if
...
method. For a complete list of supported task types, see the [Supported task types](#supported-task-types)
table.

**Note:** You cannot use the `task_type` field for the `gemini-embedding-2` model. Instead, include the task as an instruction in your prompt, as detailed in the [above section](#task-types-embeddings-2).
...
**Note:** `gemini-embedding-2` introduces automatic renormalization for non-default
dimensions.
...
* **Multiple parts (aggregated):** Adding multiple inputs directly to the
  `contents` parameter produces one aggregated embedding for all inputs.
* **Multiple `Content` objects (separate):** Wrapping each input in a
  `Content` object and passing them in the `contents` parameter returns
  separate embeddings for each entry.
...
| --- | --- |
| id\_cardModel code | **Gemini API**  `gemini-embedding-2` |
...
| 123Versions | Read the [model version patterns](/gemini-api/docs/models/gemini#model-versions) for more details.  * Stable: `gemini-embedding-2` |
| calendar\_monthLatest update | April 2026 |
...
* **Task type specification:** With `gemini-embedding-001`, you specify the
  ...
  [Task types with Embeddings 2](#task-types-embeddings-2) for details on how
...
* **Embedding aggregation:** `gemini-embedding-001` generates individual
  ...
  [Batch API](/gemini-api/docs/batch-api#batch-embedding). See
  [Embedding aggregation](#embedding-aggregation) for more information.
* **Normalization:** If you use `output_dimensionality` to request embeddings
  ...
  [Ensuring quality for smaller dimensions](#quality-for-smaller-dimensions)
  for details.
