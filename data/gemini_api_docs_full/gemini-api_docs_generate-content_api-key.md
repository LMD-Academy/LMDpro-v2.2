...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/generate-content/api-key))
...
* **Auth keys default**: All new API keys created in Google AI Studio
  are automatically created as auth keys.
...
* **On September 2026**: the Gemini API will reject requests from **Standard
  keys**. You must [migrate to auth keys](#migrate-to-auth-key)
  before this date to avoid service interruption. Make sure to migrate to
  auth keys before September 2026.

**Note:** Requests authenticated by authorization keys are not recorded in
Google Cloud service account usage metrics.
...
**Note:** To restrict an API key, you must have the `apikeys.keys.update` permission
on the associated Google Cloud project. This permission is included in roles
like **API Keys Admin** or **Editor**.
...
4. From the drop-down, select the APIs you want this key to access. Do not
   select the **Generative Language API**.
5. Click **Save**. Create a separate, restricted key in AI Studio to continue
...
**Note:** If you don't see your keys, you may need to import your Cloud projects
into AI Studio. Refer to the [import projects](#import-projects) section
for instructions.
