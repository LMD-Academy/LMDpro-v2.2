...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/generate-content/gemini-3))
...
**Note:** This version of the page covers the previous **generateContent API**. We recommend using the new [Interactions API](/gemini-api/docs/interactions-overview) for access to all the latest features and models. You can use the toggle on this page to switch to the [Interactions API version of this page](/gemini-api/docs/gemini-3).

**Deprecation notice:** This page is deprecated and will be removed.
See the [What's new
in Gemini 3.5 Flash](/gemini-api/docs/whats-new-gemini-3.5) guide for the latest developer guidance, including
migration instructions, prompting best practices, and an updated feature
overview for all Gemini 3.x models.
...
[Try Gemini 3 Flash Preview](https://aistudio.google.com/prompts/new_chat?model=gemini-3-flash-preview)
[Try Gemini 3.1 Flash-Lite](https://aistudio.google.com/prompts/new_chat?model=gemini-3-flash-lite)
...
**Note:** The `media_resolution` parameter maps to different token counts depending
on the input type. While images scale linearly (`media_resolution_low`: 280,
`media_resolution_medium`: 560, `media_resolution_high`: 1120), Video is
compressed more aggressively. For Video, both `media_resolution_low` and
`media_resolution_medium` are capped at 70 tokens per frame, and
`media_resolution_high` is capped at 280 tokens. See the full details on the
[Media resolution](/gemini-api/docs/media-resolution#token-counts) page.
...
* **Function Calling (Strict):** The API enforces strict validation on the
  ...
  **Note:** Circulation of thought signatures is required even when [thinking level](#thinking_level) is set to `minimal` for Gemini 3 Flash.
...
**Success:** If you use the [official SDKs (Python, Node, Java)](https://ai.google.dev/gemini-api/docs/function-calling?example=meeting#thinking)
and standard chat history, Thought Signatures are handled automatically. You do
not need to manually manage these fields.
...
* **Image segmentation:** Image segmentation capabilities (returning pixel-level
  ...
  utilize Gemini 2.5 Flash with thinking turned off or [Gemini Robotics-ER 1.6](/gemini-api/docs/robotics-overview).
* **Computer Use:** Gemini 3 Pro and Gemini 3 Flash support [Computer
  Use](/gemini-api/docs/computer-use). Unlike the 2.5 series, you don't need to
...
* **Tool support**: [Combining built-in tools with function calling](/gemini-api/docs/tool-combination) is now supported for Gemini 3 models. [Maps
  ...
  models.
* **Candidate count**: Gemini 3 models do not support `candidateCount > 1`.
  Setting this parameter to a value greater than `1` will return a
  400 error.
