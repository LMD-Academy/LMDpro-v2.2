...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/generate-content/google-search))
...
* `searchEntryPoint` : Contains the HTML and CSS to render the required Search
  ...
  Service](/gemini-api/terms#grounding-with-google-search).
...
context tool](/gemini-api/docs/url-context) to ground responses in both public
...
**Note:** Older models use a `google_search_retrieval` tool. For all current models,
use the `google_search` tool as shown in the examples.
...
[URL context](/gemini-api/docs/url-context) to power more complex use cases.
...
* Learn how to augment prompts with specific URLs using the [URL context
  tool](/gemini-api/docs/url-context).
