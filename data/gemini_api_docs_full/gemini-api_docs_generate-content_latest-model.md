...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/generate-content/latest-model))
...
**Note:** If you're building with the [Interactions API](/gemini-api/docs/interactions-overview), you can automate this migration with a coding agent that supports [skills](/gemini-api/docs/coding-agents#gemini-interactions-api) (like Antigravity).
...
* [Gemini 3.6 Flash model page](/gemini-api/docs/models/gemini-3.6-flash)
* [Gemini 3.5 Flash-Lite model page](/gemini-api/docs/models/gemini-3.5-flash-lite)
...
| --- | --- | --- |
| **Gemini 3.6 Flash** `gemini-3.6-flash` | Code generation, spatial/multimodal reasoning, multi-step agentic workflows | **Gemini 3.5 Flash**, **Gemini 3 Flash (Preview)**, or **Gemini 3.1 Pro** |
| **Gemini 3.5 Flash-Lite**  `gemini-3.5-flash-lite` | Autonomous subagent execution, high-volume data analysis and document extraction, structured JSON parsing | **Gemini 3.1 Flash-Lite** or **Gemini 2.5 Flash** |

## Updated Antigravity agent

Due to its improved performance, Gemini 3.6 Flash is now the new default model powering the [Antigravity agent](/gemini-api/docs/antigravity-agentn) in Gemini Managed Agents. This can be changed by setting a new field on the API.
...
If your application previously prefilled a model turn to suppress preambles or force JSON formatting, use `system_instruction` or [Structured outputs](/gemini-api/docs/structured-output) instead.
...
* **Audit function calling:**
  + Ensure all `FunctionResponse` objects include `call_id` and `name`.
  + Place multimodal assets inside the response payload.
  + Format inline instructions using `\\n\\n`.
  + If you see `Malformed_Function_Call` errors tied to pre-tool text, see [Workarounds for pre-tool text requirements](/gemini-api/docs/generate-content/function-calling#workarounds-for-pre-tool-text-requirements).
* **Baseline Gemini 3.x requirements:** For SDK updates and thought signature preservation, see the [Gemini 3.5 Migration Checklist](/gemini-api/docs/generate-content/whats-new-gemini-3.5#migration).
...
* **Remove deprecated parameters and validate function calling:** Apply the [same rules as 3.6 Flash](#migrate-to-gemini-3-6-flash).
* **Baseline Gemini 3.x requirements:** Refer to the [Gemini 3.5 Migration Checklist](/gemini-api/docs/generate-content/whats-new-gemini-3.5#migration).

## Next steps

* Review API specs on the [Models Overview](/gemini-api/docs/models).
* Explore multi-agent orchestration in the [Interactions API Guide](/gemini-api/docs/interactions).
