...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/generate-content/maps-grounding))
...
5. **Response:** The model returns a text response, which
   includes citations to Google Maps sources.
...
the [`generateContent`](/api/generate-content) method. You enable and configure
Grounding with Google Maps by including a
[`googleMaps`](/api/caching#GoogleMaps) object in the `tools` parameter of your
request.
...
includes a [`groundingMetadata`](/api/generate-content#GroundingMetadata) field.
...
[`groundingMetadata`](/api/generate-content#GroundingMetadata):
...
![Prompt with response showing sources](/static/gemini-api/docs/images/maps/sources-expanded.jpg)

You can collapse the view of the sources.

![Prompt with response and sources collapsed](/static/gemini-api/docs/images/maps/sources-collapsed.jpg)

Optional: Enhance the link preview with additional content, such as:

* A [Google Maps favicon](https://www.google.com/images/branding/product/ico/web_maps_icon_32dp.ico)
...
restrictions in the [Terms](/gemini-api/terms#grounding-with-google-maps):

* You won't use Grounding with Google Maps for high risk activities including
  emergency response services.
* You won't distribute or market your application that offers Grounding with
  Google Maps in a Prohibited Territory. For more information, see
...
* **Model Support:** See the [Supported models](#supported-models) section.
* **Multimodal Inputs/Outputs:** Grounding with Google Maps does not currently
  support multimodal inputs or outputs beyond text.
...
**Note:** The quota for Grounding with Google Maps typically aligns with the
underlying Gemini model rate limits.
...
| [Gemini 2.5 Flash](/gemini-api/docs/models/gemini-2.5-flash) | ✔️ |
