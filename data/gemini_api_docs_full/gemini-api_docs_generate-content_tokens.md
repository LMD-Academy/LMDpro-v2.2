...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/generate-content/tokens))
...
[View on ai.google.dev](https://ai.google.dev/gemini-api/docs/tokens) | [Try a Colab notebook](https://colab.research.google.com/github/google-gemini/cookbook/blob/main/quickstarts/Counting_Tokens.ipynb) | [View notebook on GitHub](https://colab.research.google.com/github/google-gemini/cookbook/blob/main/quickstarts/Counting_Tokens.ipynb) |
...
* **Call [`count_tokens`](/api/rest/v1/models/countTokens) with the input
  of the request.**  
...
* **Use the `usage_metadata` attribute on the `response` object after
  ...
  model](/gemini-api/docs/thinking), the tokens used during the thinking
  process are returned in `thoughts_token_count`. And if you are using
  [Context caching](/gemini-api/docs/caching), the cached token
...
* The separate token counts of the input (`prompt_token_count`), the cached
  content (`cached_content_token_count`) and the output
...
[Gemini 3 models](/gemini-api/docs/models#gemini-3) introduce granular control over
...
see the [media resolution](/gemini-api/docs/generate-content/media-resolution) guide.
...
**Note:** The API provides **summaries** in both the [free and paid tiers](/gemini-api/docs/pricing). **Thought signatures** increase the input token count (and your cost) when you send them back in subsequent conversational turns.
...
Thinking models generate full thoughts to improve the quality of the final response, and then output [summaries](/gemini-api/docs/thinking#summaries) to provide insight into the thought process. So, the API bases pricing on the full thought tokens the model generates to create a summary, even though the API only outputs the summary.

You can learn more about how to configure thinking in the [Gemini thinking](/gemini-api/docs/thinking) guide.
...
context window by calling the [`models.get` endpoint](/api/rest/v1/models/get)
or by looking in the [models documentation](/gemini-api/docs/models).
