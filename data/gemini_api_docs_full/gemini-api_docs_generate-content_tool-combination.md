...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/generate-content/tool-combination))
...
**Note:** This version of the page covers the previous **generateContent API**. We recommend using the new [Interactions API](/gemini-api/docs/interactions-overview) for access to all the latest features and models. You can use the toggle on this page to switch to the [Interactions API version of this page](/gemini-api/docs/tool-combination).**Preview:** Built-in and custom tools combinations are in
[Preview](https://cloud.google.com/products#product-launch-stages)
and supported for [Gemini 3](/gemini-api/docs/models#gemini-3) models only.

Gemini allows the combination of [built-in tools](/gemini-api/docs/tools), such
as `google_search`, and [function calling](/gemini-api/docs/function-calling)
...
* You must set the `include_server_side_tool_invocations` flag to `true` to
  enable tool context circulation.
* Include the [`function_declarations`](/gemini-api/docs/function-calling#function-declarations), along with the
  built-in tools you want to use, to trigger the combination behavior.
  + If you don't include `function_declarations`, tool context circulation
    will still act on the included built-in tools, as long as the flag is set.
...
* `toolCall` and `toolResponse`: The API returns these parts to preserve the
  context of which tools are run on the server side, and the result of their
  execution, for the next turn.
* `functionCall` and `functionResponse`: The API sends the function call to
  the user to fill out, and the user sends the result back in the
  function response (these parts are standard to all [function calling](/gemini-api/docs/function-calling) in the Gemini API, not unique to the
  tool combination feature).
* ([Code execution](/gemini-api/docs/code-execution) tool only)
  `executableCode` and `codeExecutionResult`:
  When using the Code Execution tool, instead of `functionCall` and
  `functionResponse`, the API returns `executableCode` (the code generated
  by the model that's meant to be executed) and `codeExecutionResult` (the
  result of the executable code).

You must return all parts, including all the [fields](#critical-fields) they
contain, back to the model on each turn to maintain context and enable tool
combinations.

### Critical fields in returned parts

Certain [parts returned by the API](#api-returns-parts) will include `id`,
...
* `id`: A unique identifier that maps a call to its response. `id` is **set on
  all function call responses**, regardless of tool context circulation.
  You *must* provide the same `id` in the function response
  that the API provides in the function call. Built-in tools automatically
  share the `id` between the tool call and tool response.
  + Found in all tool-related parts: `toolCall`, `toolResponse`,
    `functionCall`, `functionResponse`, `executableCode`, `codeExecutionResult`
* `tool_type`: Identifies the specific tool being used; the literal built-in
  tool or (e.g. `URL_CONTEXT`) or function (e.g. `getWeather`) name.
  + Found in `toolCall` and `toolResponse` parts.
* `thought_signature`: The actual encrypted context embedded in **each
  part returned by the API**. Context can't be
  reconstructed without thought signatures; if you don't return the thought
  signatures for all parts in every turn, the model will error out.
  + Found in *all* parts.
...
| **URL\_CONTEXT** | `urls` URLs to be browsed | `urls_metadata` `retrieved_url`: URLs browsed `url_retrieval_status`: Browse status |
| **FILE\_SEARCH** | None | None |
...
double-charged (see the [Pricing](/gemini-api/docs/pricing) page).

Read the [Tokens](/gemini-api/docs/tokens) page for more information.

## Limitations

* Default to `VALIDATED` mode (`AUTO` mode is not supported) when
  `include_server_side_tool_invocations` flag is enabled
* Built-in tools like `google_search` rely on location and current time
  information, so if your `system_instruction` or
  `function_declaration.description` has conflicting location and time
  information, the tool combination feature might not work well.
...
| --- | --- | --- |
| [Google Search](/gemini-api/docs/google-search) | Server-side | Supported |
| [Google Maps](/gemini-api/docs/maps-grounding) | Server-side | Supported |
| [URL Context](/gemini-api/docs/url-context) | Server-side | Supported |
| [File Search](/gemini-api/docs/file-search) | Server-side | Supported |
| [Code Execution](/gemini-api/docs/code-execution) | Server-side | Supported (built in, uses `executableCode` and `codeExecutionResult` parts) |
| [Computer Use](/gemini-api/docs/computer-use) | Client-side | Supported (built in, uses `functionCall` and `functionResponse` parts) |
| [Custom functions](/gemini-api/docs/function-calling) | Client-side | Supported (built in, uses `functionCall` and `functionResponse` parts) |

## What's next

* Learn more about [Function calling](/gemini-api/docs/function-calling) in the Gemini API.
