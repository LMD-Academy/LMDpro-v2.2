...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/imagen))
...
This model is [deprecated](/gemini-api/docs/deprecations) and will be shut down on August 17, 2026; migrate to Nano Banana for image generation.
...
**Note:** You can also generate images with Gemini's built-in multimodal
capabilities. See the [Image generation
guide](/gemini-api/docs/image-generation) for details.
...
See the [Image generation guide](/gemini-api/docs/image-generation) for more
...
* `personGeneration`: Allow the model to generate images of people. The
  ...
  + `"allow_adult"`: Generate images of adults, but not children. This is
    the default.
  + `"allow_all"`: Generate images that include adults and children.**Note:** The `"allow_all"` parameter value is not allowed in EU, UK, CH, MENA
  locations.
...
|  |  |
| --- | --- |
| Short prompts let you generate an image quickly.  Imagen 4 short prompt example   Prompt: close-up photo of a woman in her 20s, street photography, movie still, muted orange warm tones | Longer prompts let you add specific details and build your image.  Imagen 4 long prompt example   Prompt: captivating photo of a woman in her 20s utilizing a street photography style. The image should look like a movie still with muted orange warm tones. |
...
* **Multiple phrases**: Experiment with two or three distinct phrases to
  ...
  ![Imagen 4 generate text example](/static/gemini-api/docs/images/imagen/imagen3_generate-text.png)
...
1. Prompt: `A minimalist logo for a health care company on a solid color background. Include the text Journey.`
   ...
   ![Imagen 4 prompt parameterization example 1](/static/gemini-api/docs/images/imagen/imagen3_prompt-param_healthcare.png)
2. Prompt: `A modern logo for a software company on a solid color background. Include the text Silo.`

   ![Imagen 4 prompt parameterization example 2](/static/gemini-api/docs/images/imagen/imagen3_prompt-param_software.png)
3. Prompt: `A traditional logo for a baking company on a solid color background. Include the text Seed.`

   ![Imagen 4 prompt parameterization example 3](/static/gemini-api/docs/images/imagen/imagen3_prompt-param_baking.png)
...
| --- | --- |
| id\_cardModel code | **Gemini API**  `imagen-4.0-generate-001`  `imagen-4.0-ultra-generate-001`  `imagen-4.0-fast-generate-001` |
| saveSupported data types | **Input**  Text  **Output**  Images |
| token\_autoToken limits[[\*]](/gemini-api/docs/tokens) | **Input token limit**  480 tokens (text)  **Output images**  1 to 4 (Ultra/Standard/Fast) |
