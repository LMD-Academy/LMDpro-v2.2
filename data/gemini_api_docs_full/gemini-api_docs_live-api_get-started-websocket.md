...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/live-api/get-started-websocket))
...
[Use coding agent skillsterminal](/gemini-api/docs/coding-agents)
...
* **Message Format**: All communication is done via JSON messages conforming to [`BidiGenerateContentClientMessage`](/api/live#bidigeneratecontentclientmessage) and [`BidiGenerateContentServerMessage`](/api/live#bidigeneratecontentservermessage) structures.
...
If you are using [ephemeral tokens](/gemini-api/docs/ephemeral-tokens), you need to connect to the `v1beta` endpoint.
...
The first message sent over the WebSocket must be a [`BidiGenerateContentSetup`](/api/live#bidigeneratecontentsetup) containing the `config`.
For the full configuration options, see the [Live API - WebSockets API reference](/api/live).
...
To send text input, construct a [`BidiGenerateContentRealtimeInput`](/api/live#bidigeneratecontentrealtimeinput) message with the `text` field.
...
Audio needs to be sent as raw PCM data (raw 16-bit PCM audio, 16kHz, little-endian). Construct a [`BidiGenerateContentRealtimeInput`](/api/live#bidigeneratecontentrealtimeinput) message with the audio data. The `mimeType` is crucial.
...
The WebSocket will send back [`BidiGenerateContentServerMessage`](/api/live#bidigeneratecontentservermessage) messages. You need to parse these JSON messages and handle different types of content.
...
When the model requests a tool call, the [`BidiGenerateContentServerMessage`](/api/live#bidigeneratecontentservermessage) will contain a `toolCall` field. You must execute the function locally and send the result back to the WebSocket using a [`BidiGenerateContentToolResponse`](/api/live#bidigeneratecontenttoolresponse) message.
