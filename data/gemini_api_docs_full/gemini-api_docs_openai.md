...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/openai))
...
* **`base_url="https://generativelanguage.googleapis.com/v1beta/openai/"`:** This
  ...
  the default URL.
* **`model="gemini-3.6-flash"`**: Choose a compatible Gemini model
...
parameters](/gemini-api/docs/thinking) which give fine grain
...
default [level](/gemini-api/docs/thinking#levels) or [budget](/gemini-api/docs/thinking#set-budget).
...
Gemini thinking models also produce [thought summaries](/gemini-api/docs/thinking#summaries).
You can use the [`extra_body`](#extra-body) field to include Gemini fields
in your request.
...
APIs. You can find the full example on the [thought signatures](/gemini-api/docs/thought-signatures#openai) page.
...
Generate an image using `gemini-2.5-flash-image` or `gemini-3-pro-image-preview`. Supported parameters include `prompt`, `model`, `n`, `size`, and `response_format`. Any other parameters not listed here or in the [`extra_body`](#extra-body) section will be silently ignored by the compatibility layer.

You can enable **Grounding with Google Search** and configure **safety settings**
using the `extra_body` parameter. See the [`extra_body`](#extra-body) section
...
`/v1/videos` endpoint. Supported top-level parameters are `prompt` and `model`. Additional parameters like `duration_seconds`, `image`, and `aspect_ratio` must be passed with `extra_body`. See the [`extra_body`](#extra-body) section
...
using the [Gemini API](/gemini-api/docs/embeddings). You can use
...
You can create [batch jobs](/gemini-api/docs/batch-mode), submit them, and check
...
[files](/gemini-api/docs/files), the same as when using the Gemini [Batch API](/gemini-api/docs/batch-mode#input-file).
...
The OpenAI SDK also supports [generating embeddings with the Batch API](/gemini-api/docs/batch-api#batch-embeddings). To do so, switch out the
...
Learn more about inference tiers in the [Optimization](/gemini-api/docs/optimization) documentation.
