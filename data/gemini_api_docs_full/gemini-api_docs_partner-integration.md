...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/partner-integration))
...
**Caution:** If you are building an end-user application (e.g., a customer support
bot, a travel planner), skip this guide and use the [Gemini API
Get started guide](/gemini-api/docs/get-started). The [GenAI
SDK](/gemini-api/docs/libraries) is the recommended choice for you.
...
header](#client-id) regardless of the path chosen.

| If you are... | Recommended path | Key benefit | Key trade-off | Best practice |
| --- | --- | --- | --- | --- |
| **Enterprise gateway, ecosystem framework** | **[Google GenAI SDK](#genai-sdk)** | **Gemini Enterprise Agent Platform parity & speed.** Built-in handling for types, auth, and complex features (e.g., file uploads). Seamless migration to Google Cloud. | **Dependency weight.** Transitive dependencies can be complex and outside of your control. Limited to supported languages (Python/Node/Go/Java). | **Lock versions.** Pin SDK versions in your internal base images to ensure stability across teams. |
| **Ecosystem framework, edge platforms, and aggregators** | **[Direct API](#rest)**  *(REST / gRPC)* | **Zero dependencies.** You control the HTTP client and exact bundle size. Full access to all API and model features. | **High developer overhead.** JSON structures can be deeply nested and require strict manual validation and type-checking. | **Use OpenAPI specs.** Automate type generation using our official specs rather than writing them by hand. |
| **Aggregator using OpenAI SDKs that only require text-based workflows**  *(Optimizing for legacy portability)* | **[OpenAI compatibility](#openai)** | **Instant portability.** Reuse existing OpenAI-compatible code or libraries. | **Feature ceiling.** Model-specific features (Native video, Caching) may not be available. | **Migration plan.** Use this for rapid validation, but plan to upgrade to Direct API for complete API feature. |

## Google GenAI SDK integration

For frameworks, implementing the [Google GenAI SDK](/gemini-api/docs/libraries)
...
* **Unified interface for Gemini Enterprise Agent Platform migration:** Internal developers often
...
* **Client-side helpers:** The SDK includes idiomatic utilities that reduce
  ...
  + *Examples:* Supporting `PIL` image objects directly in prompts,
...
* **Language support:** The SDKs support *current* language versions. If you
...
* **Manual Gemini Enterprise Agent Platform implementation:** Unlike the SDK, using the API directly
...
* Visit the [library overview](/gemini-api/docs/libraries) to learn about
  the GenAI SDKs
* Browse the [API reference](/api)
* Read the [OpenAI compatibility guide](/gemini-api/docs/openai)
