...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/realtime-music-generation))
...
**Experimental:** Lyria RealTime is an
[experimental model](/gemini-api/docs/models#experimental).
...
**Note:** Looking for non-streaming music generation? See
[Generate music with Lyria 3](https://ai.google.dev/gemini-api/docs/music-generation).

## Generate and control music

Lyria RealTime works similarly to the [Live API](https://ai.google.dev/gemini-api/docs/live-api)
...
**Objective:** For a more complete code sample, refer to the
[Lyria RealTime - Get Started](https://colab.sandbox.google.com/github/google-gemini/cookbook/blob/main/quickstarts/Get_started_LyriaRealTime.ipynb)
...
**Objective:** For a more complete code sample, try the
[Prompt DJ](https://aistudio.google.com/apps/bundled/promptdj)
and [MIDI DJ](https://aistudio.google.com/apps/bundled/promptdj-midi) apps on
AI Studio
...
You can steer music generation in real-time by sending prompts and updating
...
You can steer music generation by updating the music generation parameters in
...
* `WeightedPrompt`: A text string describing a musical idea, genre, instrument,
  ...
  influences. See [above](#steer-music) for more details on how to best prompt
  Lyria RealTime.
* `MusicGenerationConfig`: Configuration for the music generation process,
  ...
  + `scale`: (Enum)
    ...
    [`Scale` enum values](#scale-enum) provided by the SDK. You need to
    stop/play or reset the context for the model it take into account the new
    scale.
...
* Instead of music, learn how to generate multi-speakers conversation using
  the [TTS models](https://ai.google.dev/gemini-api/docs/speech-generation),
