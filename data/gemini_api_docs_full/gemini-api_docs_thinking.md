...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/thinking))
...
| `summary` | ❌ No | An array of content (text and/or images) summarizing the reasoning. May be empty depending on the [`thinking_summaries`](/api/interactions-api) config, whether the model performed enough reasoning, or the content type (for example, image latents may not have text summaries). |
