...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemini-api/docs/troubleshooting))
...
your API key correctly per the [API key setup guide](/gemini-api/docs/api-key).
...
[API errors](/gemini-api/docs/api-errors) page.
...
| Temperature | 0.0-1.0 |
| Max output tokens | Use the [models page](/gemini-api/docs/models/gemini) to determine the maximum number of tokens for the model you are using. |
| TopP | 0.0-1.0 |
...
page](/gemini-api/docs/models/gemini).
...
Refer to [thinking page](/gemini-api/docs/thinking#set-budget) for
...
Gemini API. You can use [Google AI Studio](/gemini-api/docs/api-keys) to see if any of
...
AI Studio](/gemini-api/docs/api-keys). We strongly recommend reviewing your API
...
* We are moving towards issuing API keys when you request a new key using
  [Google AI Studio](/gemini-api/docs/api-keys) that will by default be
...
* You will be able to find the status of your API keys within [Google AI
  Studio](/gemini-api/docs/api-keys) and we will work on communicating
...
[prompt engineering guide](/gemini-api/docs/prompting-strategies) page
...
* The API supports only a number of select languages. Submitting prompts in
  ...
  [available languages](/gemini-api/docs/models#supported-languages) for
  updates.
