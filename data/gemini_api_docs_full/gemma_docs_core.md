...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemma/docs/core))
...
[Model Card](/gemma/docs/core/model_card_4) and
...
* **Extended Multimodalities:** Processes Text,
  [Image](/gemma/docs/capabilities/vision/image) with variable aspect ratio
  and resolution support (all models),
  [Video](/gemma/docs/capabilities/vision/video), and
  [Audio](/gemma/docs/capabilities/audio) (featured natively on the E2B, E4B
  and 12B models).
...
* **[Multi-Token Prediction](/gemma/docs/mtp/overview):** All Gemma 4 models
...
| Parameters | BF16 (16-bit) | SFP8 (8-bit) | Q4\_0 (4-bit) | Mobile | Mobile (Text-only) |
| --- | --- | --- | --- | --- | --- |
...
| vLLM / SGLang | SERVER: `{model-name}-qat-w4a16-ct` MOBILE: `{model-name}-qat-mobile-ct` | High-throughput inference utilizing 4-bit weights with 16-bit activations. |
| Speculative Decoding | MODEL: `{model-name}-qat-q4_0-unquantized` DRAFTER: `{model-name}-qat-q4_0-unquantized-assistant` | Running a primary model alongside its matching MTP draft model to drastically accelerate token generation. The model must be quantized. |
...
* Gemma 3 [Model Card](/gemma/docs/core/model_card_3)
