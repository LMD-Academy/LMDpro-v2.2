...
[//]: # (source: [ai.google.dev](https://ai.google.dev/gemma/docs/paligemma))
...
[Gemma models list](/gemma/docs/get_started#models-list).
...
* Download from
  [Kaggle](https://www.kaggle.com/models?query=paligemma).
* Download from
  [Hugging Face](https://huggingface.co/models?search=google/paligemma).

**Important:** **Pretrained** PaliGemma models *require* tuning in order to
produce useful results. Make sure you perform fine-tuning on these PaliGemma
models and test the output *before* deploying them to end users.

There are three categories of PaliGemma models:

* **PaliGemma PT** - General purpose pre-trained models that can be fine-tuned on a
  variety of tasks.
* **PaliGemma FT** - Research-oriented models that are fine-tuned on specific
  research datasets.
* **PaliGemma mix** - Models tuned to a mixture of tasks that can be used
...
### [Run in Colab](https://ai.google.dev/gemma/docs/paligemma/inference-with-keras)

Try detection and content generation capabilities with PaliGemma in Colab.

### [Tune in Colab](https://ai.google.dev/gemma/docs/paligemma/fine-tuning-paligemma)
